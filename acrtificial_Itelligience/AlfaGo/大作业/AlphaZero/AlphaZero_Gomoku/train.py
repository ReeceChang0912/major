...
class TrainPipeline():
    def __init__(self, init_model=None):
        ...
        self.lr_multiplier = 1.0  # adaptively adjust the learning rate based on KL 适当的调整学习率以KL为基础
        self.temp = 1.0 # the temperature param 温度参数
        self.n_playout = 400 # num of simulations for each move每次的模仿
        self.c_puct = 5
        self.buffer_size = 10000
        self.batch_size = 512 # mini-batch size for training 训练的最小堆大小
        self.data_buffer = deque(maxlen=self.buffer_size)        
        self.play_batch_size = 1 
        self.epochs = 5 # num of train_steps for each update每次更新的训练的步长
        self.kl_targ = 0.025
        self.check_freq = 50  #50
        self.game_batch_num = 10
        self.best_win_ratio = 0.0
        # num of simulations used for the pure mcts, which is used as the opponent to evaluate the trained policy用来评估对手的策略
        self.pure_mcts_playout_num = 1000  
        if init_model:
            # start training from an initial policy-value net从一个初始的策略值开始训练
            ...
            # start training from a new policy-value net 开始训练一个新的策略网络
    ...
    def collect_selfplay_data(self, n_games=1):
        """collect self-play data for training"""  #收集自己玩的数据来继续训练
    ...
    def policy_update(self):
        """update the policy-value net"""   #更新策略网络的值
        ...
        for i in range(self.epochs): 
            ...
            if kl > self.kl_targ * 4:   # early stopping if D_KL diverges badly  #如果发散的很厉害就早停止
                break
    ...
    def run(self):
        ...
        try:
            for i in range(self.game_batch_num):                
                ...
                #check the performance of the current model锛宎nd save the model params  #检查现在模型的表现
                if (i+1) % self.check_freq == 0:
                    print("current self-play batch: {}".format(i+1))
                    win_ratio = self.policy_evaluate(batch= i+1)
                    net_params = self.policy_value_net.get_policy_param() # get model params 得到模型参数
